...
!
      ...
      !----< adios_write() >------------------------------------------
      subroutine adios_write_s3d(filename)
          ...
          integer dimids(4), req(4), st(4), err, adios_err, cmode
          integer i, yspecies_id, u_id, pressure_id, temp_id
          ...
          call adios_init ("s3dio.xml", gcomm, adios_err);
          call adios_open (adios_handle, "s3dio", filename, "w", gcomm, adios_err);
...
#include "gwrite_s3dio.fh"
      ...
      !----< adios_read() >-------------------------------------------
      subroutine adios_read_s3d(filename)
        use topology_m,  only : gcomm, npes, mypx, mypy, mypz
        use param_m,     only : nx, ny, nz, nx_g, ny_g, nz_g, nsc
        use variables_m, only : temp, pressure, yspecies, u
        use runtime_m,   only : method, time, tstep, time_save, io_one_species_at_a_time
