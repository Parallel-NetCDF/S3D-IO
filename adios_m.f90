...
!
      ...
      !----< adios_write() >------------------------------------------
      subroutine adios_write(filename)
          ...
          call adios_init ("s3dio.xml", gcomm, err);
          call adios_open (adios_handle, "arrays", filename, "w", comm, v);
      #include "gwrite_arrays.fh"
          call adios_close (adios_handle, err)
      
          call MPI_Barrier (gcomm, err);
      
          call adios_finalize (myid, err);
      ...
      !----< adios_read() >-------------------------------------------
      subroutine adios_read(filename)
          use topology_m,  only : gcomm, npes, mypx, mypy, mypz, myid
          use param_m,     only : nx, ny, nz, nx_g, ny_g, nz_g, nsc
          use variables_m, only : temp, pressure, yspecies, u
          use runtime_m,   only : method, time, tstep, time_save, io_one_species_at_a_time
